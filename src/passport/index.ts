...
passport.use(
  ...
  new Strategy(
    ...
    async function verify(req, username, password, done) {
      ...
      if (user) {
        return done("That login is already taken.", false, {
...
passport.use(
  ...
  new Strategy(
    ...
    async function verify(req, username, password, done) {
      ...
      if (user) {
        return done(
          "TThe administrator is already there. It is forbidden to create another admin",
          false,
          {
            message:
              "The administrator is already there. It is forbidden to create another admin",
          }
        );
      } else {
        bcrypt.hash(password, salt, async (err: Error, hash: string) => {
