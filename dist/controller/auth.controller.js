"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.setAdminNewPassword = exports.getAdminUserData = exports.passportAdminRegistration = exports.passportAdminAuthenticate = exports.adminRegistration = exports.setNewPassword = exports.getUserData = exports.passportRegistration = exports.passportAuthenticate = exports.logOut = exports.registration = void 0;
var bcryptjs_1 = __importDefault(require("bcryptjs"));
var user_service_1 = require("../service/user.service");
var index_1 = __importDefault(require("../passport/index"));
var user_model_1 = require("../../src/model/user.model");
var registration = function (req, res, next) {
    var salt = +process.env.SALT;
    bcryptjs_1.default.hash(req.body.password, salt, function (err, hash) { return __awaiter(void 0, void 0, void 0, function () {
        var result, e_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, (0, user_service_1.saveUser)(req.body.username, hash)];
                case 1:
                    result = _a.sent();
                    res.send(result);
                    return [3 /*break*/, 3];
                case 2:
                    e_1 = _a.sent();
                    res.status(503);
                    res.end(e_1);
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    }); });
};
exports.registration = registration;
var logOut = function (req, res, next) {
    req.logout({ keepSessionInfo: false }, function (err) {
        console.log(err);
        if (err) {
            return next(err);
        }
        res.status(200);
        res.end();
    });
};
exports.logOut = logOut;
exports.passportAuthenticate = index_1.default.authenticate("local", {
    successMessage: true,
    failureMessage: true,
});
exports.passportRegistration = index_1.default.authenticate("local-signup", {
    successMessage: true,
    failureMessage: true
});
var getUserData = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        try {
            if (!req.user) {
                res.send("Unauthorized");
            }
            else {
                res.send(req.user);
            }
        }
        catch (e) {
            res.status(503);
            res.end(e);
        }
        return [2 /*return*/];
    });
}); };
exports.getUserData = getUserData;
var setNewPassword = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user, e_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 4, , 5]);
                if (!req.user) return [3 /*break*/, 2];
                return [4 /*yield*/, user_model_1.User.findById(req.body.id)];
            case 1:
                user = _a.sent();
                bcryptjs_1.default.compare(req.body.old_password, user === null || user === void 0 ? void 0 : user.password, function (err, result) {
                    if (result) {
                        var salt = +process.env.SALT;
                        bcryptjs_1.default.hash(req.body.new_password, salt, function (err, hash) { return __awaiter(void 0, void 0, void 0, function () {
                            var _a, _b;
                            return __generator(this, function (_c) {
                                switch (_c.label) {
                                    case 0:
                                        console.log(hash);
                                        _b = (_a = res).send;
                                        return [4 /*yield*/, (0, user_service_1.ResetPassword)(req.body.id, hash)];
                                    case 1:
                                        _b.apply(_a, [_c.sent()]);
                                        return [2 /*return*/];
                                }
                            });
                        }); });
                    }
                });
                return [3 /*break*/, 3];
            case 2: throw Error("Unauthorized");
            case 3: return [3 /*break*/, 5];
            case 4:
                e_2 = _a.sent();
                res.status(503);
                res.end(e_2.message);
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); };
exports.setNewPassword = setNewPassword;
var adminRegistration = function (req, res, next) {
    var salt = +process.env.SALT;
    bcryptjs_1.default.hash(req.body.password, salt, function (err, hash) { return __awaiter(void 0, void 0, void 0, function () {
        var hasAdmin, result, e_3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    return [4 /*yield*/, user_model_1.Admin.findOne()];
                case 1:
                    hasAdmin = _a.sent();
                    if (hasAdmin === null || hasAdmin === void 0 ? void 0 : hasAdmin.id) {
                        throw new Error("The administrator is already there. It is forbidden to create another admin");
                    }
                    return [4 /*yield*/, (0, user_service_1.saveAdmin)(req.body.username, hash)];
                case 2:
                    result = _a.sent();
                    res.send(result);
                    return [3 /*break*/, 4];
                case 3:
                    e_3 = _a.sent();
                    res.status(503);
                    res.end(e_3);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    }); });
};
exports.adminRegistration = adminRegistration;
exports.passportAdminAuthenticate = index_1.default.authenticate('local-admin', {
    successMessage: true,
    failureMessage: true
});
exports.passportAdminRegistration = index_1.default.authenticate("local-signup-admin", {
    successMessage: true,
    failureMessage: true
});
var getAdminUserData = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        try {
            if (!req.user) {
                res.send("Unauthorized");
            }
            else {
                res.send(req.user);
            }
        }
        catch (e) {
            res.status(503);
            res.end(e);
        }
        return [2 /*return*/];
    });
}); };
exports.getAdminUserData = getAdminUserData;
var setAdminNewPassword = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user, e_4;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 4, , 5]);
                if (!req.user) return [3 /*break*/, 2];
                return [4 /*yield*/, user_model_1.Admin.findById(req.body.id)];
            case 1:
                user = _a.sent();
                bcryptjs_1.default.compare(req.body.old_password, user === null || user === void 0 ? void 0 : user.password, function (err, result) {
                    if (result) {
                        var salt = +process.env.SALT;
                        bcryptjs_1.default.hash(req.body.new_password, salt, function (err, hash) { return __awaiter(void 0, void 0, void 0, function () {
                            var _a, _b;
                            return __generator(this, function (_c) {
                                switch (_c.label) {
                                    case 0:
                                        console.log(hash);
                                        _b = (_a = res).send;
                                        return [4 /*yield*/, (0, user_service_1.ResetPasswordAdmin)(req.body.id, hash)];
                                    case 1:
                                        _b.apply(_a, [_c.sent()]);
                                        return [2 /*return*/];
                                }
                            });
                        }); });
                    }
                });
                return [3 /*break*/, 3];
            case 2: throw Error("Unauthorized");
            case 3: return [3 /*break*/, 5];
            case 4:
                e_4 = _a.sent();
                res.status(503);
                res.end(e_4.message);
                return [3 /*break*/, 5];
            case 5: return [2 /*return*/];
        }
    });
}); };
exports.setAdminNewPassword = setAdminNewPassword;
//# sourceMappingURL=auth.controller.js.map